    ...
    def attention(self, x: torch.Tensor):
        ...
        if self.attn_mask is not None :
            if self.attn_mask.shape[0] == seq_len:
                ...
                # attn_mask = torch.zeros(seq_len, seq_len, dtype=x.dtype, device=x.device)
        ...
        #     # 1. 创建一个全为 0 的注意力掩码（默认不屏蔽任何位置）
        #     attn_mask = torch.zeros(seq_len, seq_len, dtype=x.dtype, device=x.device)
        
        #     # 2. 随机设置一部分位置为 -inf，比例设为 30%（可调）
        #     mask_ratio = 0.3
        #     num_mask = int(mask_ratio * seq_len * seq_len)

        #     # 3. 随机选择要屏蔽的位置索引
        #     rand_indices = torch.randint(0, seq_len * seq_len, (num_mask,), device=x.device)
        #     row_indices = rand_indices // seq_len
        #     col_indices = rand_indices % seq_len
        #     # 4. 设置这些位置为 -inf
        #     attn_mask[row_indices, col_indices] = float('-inf')
        #     self.attn_mask = attn_mask.to(dtype=x.dtype, device=x.device) 

        # print(f"attn_mask.shape:{attn_mask.shape}\n")
        # print(f"attn_mask:{attn_mask}\n")
        # print(f"self.attn_mask:{self.attn_mask}\n")
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.ln_post(x[:, 0, :]) #预测： NLD -> NLD, 这里是按照 N 逐个进行的进行的么？即μ和σ都已经确定了，只是简单的计算？

        if self.proj is not None: # 但是这里有一个class_embedding,
            x = x @ self.proj
...
class CLIP(nn.Module):
    ...
    def build_attention_mask(self):
        ...
        # mask = torch.zeros(self.context_length, self.context_length)
