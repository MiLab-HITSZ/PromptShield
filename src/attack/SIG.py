...
class SIG():
    ...
    def apply_trigger(self, image):
        ...
        def sig(img, delta, freq):
            ...
            # print(f"type(overlay):{type(overlay)},type(image):{type(image)}")

            overlay = 0.2 * overlay + 0.8 * img
            overlay = np.clip(overlay, 0, 255).astype(np.uint8)
            return overlay  
        
        delta = self.attack_config["delta"]
        frequency = self.attack_config["frequency"]

        img_arr = (np.array(image) * 255).astype(np.uint8) 
        if image.shape[0] == 3:  
            img_arr = img_arr.transpose(1, 2, 0)

        img_arr = sig(img_arr, delta, frequency)
